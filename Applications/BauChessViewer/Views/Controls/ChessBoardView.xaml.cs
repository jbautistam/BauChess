	...
	public partial class ChessBoardView : UserControl
	{
		...
		private void ChangeImages()
		{
			foreach (FrameworkElement control in udtCanvas.Children)
				if (control != null && control is Image image && control.Tag != null && control.Tag is ScapeBaseViewModel scape)
					image.Source = LoadImage(GetImageFileName(scape));
		}
		...
		private string GetImageFileName(ScapeBaseViewModel scape)
		{
				...
				switch (scape)
				{
					case CellViewModel cell:
						return System.IO.Path.Combine(ViewModel.MainViewModel.ComboPathBoard.GetFullPathName(), cell.FileImage);
					case FigureViewModel figure:
						return System.IO.Path.Combine(ViewModel.MainViewModel.ComboPathPieces.GetFullPathName(), figure.FileImage);
				}
				// Si ha llegado hasta aquí es porque el scape no pertencía a ningún dato válido
				return "";
		}
